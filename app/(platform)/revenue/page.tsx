...
export default async function RevenuePage() {
  try {
    ...
    const overheadThisMonth = overheadResult.data as OverheadPerJobMonth | null;
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <section className="mt-6 rounded-3xl border border-[#e3ded1] bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-semibold text-[#6b705c]">
                  Overhead Cost Per Job (this month)
                </p>

                <p className="mt-1 text-xs text-[#6b705c]">
                  Recurring and amortized overhead, divided across this
                  month's job count.{" "}
                  <Link
                    href="/costs"
              ...
              overheadThisMonth.overhead_per_job !== null ? (
                <div className="text-right">
                  ...
                  <p className="mt-1 text-sm text-[#6b705c]">
                    ...
                    total ÷{" "}
                    {formatNumber(toNumber(overheadThisMonth.job_count))}{" "}
                    jobs
                  </p>
                </div>
              ) : (
