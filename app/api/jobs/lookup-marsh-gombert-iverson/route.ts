// One-time diagnostic (2026-09-22). Same pattern used to find Baldriche
// and Katie Ray before their flips -- before setting a new customer's
...
// titles (to match her own naming convention for Full cleanings, same
// "<Name> - Full - Monthly" / "<Name> - Maintenance - Monthly"
// convention Baldriche and Ray already confirmed).
//
// Name match is widened deliberately (full_name/first_name/last_name
// checked independently per person) after the Baldridge/Baldriche
// misspelling mistake -- a narrow exact-match filter can silently
// return nothing and look like "no customer found" instead of "wrong
// spelling". Covers all three requested customers in one call.
//
...
export async function GET() {
  ...
  const { data: customers, error: customerError } = await supabaseServer
    ...
    .or(
      [
        "full_name.ilike.%marsh%",
        "first_name.ilike.%emily%",
        "last_name.ilike.%marsh%",
        "full_name.ilike.%gombert%",
        "first_name.ilike.%sarah%",
        "last_name.ilike.%gombert%",
        "full_name.ilike.%iverson%",
        "first_name.ilike.%linda%",
        "last_name.ilike.%iverson%",
      ].join(",")
    );
