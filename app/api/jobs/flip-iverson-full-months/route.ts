...
// scoped to Linda Iverson only -- hardcoded to exactly one
// jobberClientId on purpose, same pattern as Baldriche/Ray/Gombert/
// Emily Marsh.
//
// Highest-confidence case so far: her own most recent completed visits
// (confirmed via lookup-marsh-gombert-iverson) already read exactly
// "Iverson - Full - Monthly" (Sep 18) and "Iverson - Maintenance -
// Monthly" (Aug 14, Jul 18) -- this isn't an inferred convention, it's
// her actual historical titling, directly confirming both target
// titles below. Only her upcoming, not-yet-occurred visits (all still
// the generic "Iverson - Monthly Maintenance Plan") need retitling.
//
// Does two things, both scoped to her:
//   1. Sets customers.full_cleaning_months to [3,6,9,12] (Mar/Jun/Sep/
//      Dec), matching her own Service Instructions text: "Full
//      Cleaning - Mar, June, Sept, Dec".
//   2. Retitles her future visits: those in her Full months to
//      "Iverson - Full - Monthly", every other future visit to
//      "Iverson - Maintenance - Monthly".
//
...
const IVERSON_CLIENT_ID = "Z2lkOi8vSm9iYmVyL0NsaWVudC8xMDE2NTc3ODM=";
const FULL_MONTHS = [3, 6, 9, 12];
const FULL_TITLE = "Iverson - Full - Monthly";
const MAINTENANCE_TITLE = "Iverson - Maintenance - Monthly";
...
export async function GET(request: Request) {
  ...
  const { data: customer, error: customerError } = await supabaseServer
    ...
    .eq("jobber_client_id", IVERSON_CLIENT_ID)
    .maybeSingle();
