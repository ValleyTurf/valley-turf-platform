...
// scoped to Kaleen Carter only -- hardcoded to exactly one
// jobberClientId on purpose (not Kelly Carter, who also matched the
// name search used to find her -- her jobber_client_id is never
// referenced here). Does two things, both scoped to her:
...
//      months (currently the generic "Carter - Monthly Maintenance
//      Plan") to "Carter - Full - Monthly", and every other future
//      visit (same generic title) to "Carter - Maintenance -
//      Monthly".
//
...
const CARTER_CLIENT_ID = "Z2lkOi8vSm9iYmVyL0NsaWVudC8xMDkxMTAzODA=";
const FULL_MONTHS = [3, 6, 9, 12];
const FULL_TITLE = "Carter - Full - Monthly";
const MAINTENANCE_TITLE = "Carter - Maintenance - Monthly";
...
export async function GET(request: Request) {
  ...
  const { data: customer, error: customerError } = await supabaseServer
    ...
    .eq("jobber_client_id", CARTER_CLIENT_ID)
    .maybeSingle();
